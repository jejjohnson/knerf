...
2. **Lint** – `uv run --group lint ruff check src/knerf/` (or `make lint`) must report no issues.
3. **Format** – `uv run --group lint ruff format --check src/knerf/` must report no files to reformat.
4. **Type checks** – `uv run --group typecheck ty check src/knerf` (or `make typecheck`) must report no errors in changed files.
